...
import threading
from std_msgs.msg import String, Float32MultiArray  # Import String message type
    ...
    try:
        while True:
            if user_input == "1":
                ...
                # -----------------------------------------------------
                target_info_data = [0.0, 0.0, 0.0]
                parsed_offsets_3d = json.loads(offsets_3d)
                for obj in parsed_offsets_3d:
                    if obj["label"] == "pikachu":
                        target_info_data[0] = 1.0 # Found target
                        target_info_data[1] = float(obj["offset_flu"][0])
                        target_info_data[2] = float(-obj["offset_flu"][1])
                        break
                target_info_msg = Float32MultiArray()
                target_info_msg.data = target_info_data
                ros_communicator.publish_data("target_info", target_info_msg)
                # -------------------------------------------------------
                if camera_geometry.get_x_multi_depths() is not None:
                    x_multi_depths_msg = Float32MultiArray()
                    x_multi_depths_msg.data = camera_geometry.get_x_multi_depths()
                    ros_communicator.publish_data("x_multi_depths", x_multi_depths_msg)
