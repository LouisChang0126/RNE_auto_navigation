...
class YoloDepthExtractor:
    ...
    def get_depth_camera_x_value(self):
        """
        Returns the depth value at Y 座標在畫面中心,

        Returns:
            dict: Contains 'depth' values array.
                  Returns None if depth image is invalid.
        """
        self.x_num_splits = 21
        ...
        points_x = [i * segment_length for i in range(self.x_num_splits)]
        x_depth_values = [depth_cv_image[center_y, x] for x in points_x]

        x_depth_values = [float(0.0) if np.isnan(x) else float(x) for x in x_depth_values]
        # for i in range(self.x_num_splits):
        #     if x_depth_values[i] <= 0 or np.isnan(x_depth_values[i]):
        #         window_size = 2  # Fixed window size (e.g., 5x5) for camera center
        #         min_r, max_r = max(0, center_y - window_size), min(
        #             height, center_y + window_size + 1
        #         )
        #         min_c, max_c = max(0, points_x[i] - window_size), min(
        #             width, points_x[i] + window_size + 1
        #         )
        #         window = depth_cv_image[min_r:max_r, min_c:max_c]

        #         non_zero_values = window[(window > 0) & (~np.isnan(window))]
        #         if non_zero_values.size > 0:
        #             x_depth_values[i] = float(np.mean(non_zero_values))
        #         else:
        #             print(f"No valid depth value found near the {i}th X coordinate.")
        #             x_depth_values[i] = 0.0
        ...
        #     print("Final center depth is NaN.")
        #     return None  # Return None if depth remains NaN
        print(f"Depth values at X coordinates: {x_depth_values}")
        return x_depth_values
